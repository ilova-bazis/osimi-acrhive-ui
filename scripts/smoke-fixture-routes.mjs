...
export const FILE_UPLOAD = 'file-um98-upload';
...
export const RESYNC_REQUEST_ID = 'request-um98-resync';
...
export const sendBytes = (response, bytes, contentType, request) => {
	const etag = etagFor(bytes);
	const ifRange = request.headers['if-range'];
	const rangeHeader = request.headers.range;
	const rangeApplicable = (ifRange === undefined || ifRange === etag) && Boolean(rangeHeader);
	if (rangeApplicable) {
...
export const buildRoutes = (context) => [
	{
		method: 'GET',
		pattern: /^\/healthz$/,
		auth: false,
	...
	},
	{
		...
		handler: async (_request, response) => {
			...
			context.presignedUploads.add(FILE_UPLOAD);
			json(response, 201, {
				file_id: FILE_UPLOAD,
				storage_key: `smoke/upload/${FILE_UPLOAD}`,
				upload_url: `http://127.0.0.1:${process.env.SMOKE_FIXTURE_PORT ?? 4601}/smoke-upload/${FILE_UPLOAD}`,
	...
	},
	{
		...
		handler: async (request, response) => {
			const body = await readJsonBody(request);
			if (!body.file_id || !context.presignedUploads.has(body.file_id)) {
				json(response, 400, {
			...
			context.committedUploads.add(body.file_id);
			json(response, 200, {
				file: { file_id: body.file_id, status: 'UPLOADED' }
			});
		}
	},
	{
	...
	},
	{
		...
		handler: async (request, response) => {
			...
			if (actionType === 'curation_apply') {
				const matchesTarget =
					(!targetType || targetType === 'object') && (!targetId || targetId === DOC_OBJECT_ID);
				requests = matchesTarget ? [publicationRequest('COMPLETED')] : [];
			} else if (activeOnly === 'true' && targetId === AUD_OBJECT_ID) {
				requests = [
					{
	...
	},
	{
		...
		handler: async (request, response) => {
			...
			if (!context.presignedUploads.has(fileId)) {
				json(response, 404, {
