<script lang="ts">
    ...
    import type {
        ...
        IngestionMediaKind,
    } from "$lib/services/ingestionCapabilities";
    ...
    } from "$lib/services/ingestionDetail";
    ...
    import {
        ...
        resolveBatchIntent,
        type BatchIntent,
    } from "$lib/ingestion/batchIntent";
    import {
        classificationTypes,
    ...
    const groupedFileIds = $derived(
        new Set(objectGroups.flatMap((g) => g.fileIds)),
    );
    const standaloneFiles = $derived(
        files.filter((f) => !groupedFileIds.has(f.id)),
    );
    ...
    const readSummaryDate = (rawDate: unknown): SummaryDate => {
        ...
        return {
            value:
                typeof date.value === "string" || date.value === null
                    ? date.value
                    : null,
            approximate:
                typeof date.approximate === "boolean"
                    ? date.approximate
                    : false,
            confidence:
                confidence === "low" ||
                ...
                confidence === "medium"
                    ...
                    : "medium",
            note:
                typeof date.note === "string" || date.note === null
                    ? date.note
                    : null,
        };
    };

    const readSummaryDates = (
        summary: Record<string, unknown>,
    ): SummaryDates => {
    ...
    const toSummaryDateValue = (
        editor: SummaryDateEditor,
        label: string,
    ): string | null => {
        ...
        if (editor.precision === "year") {
            if (!/^\d{4}$/.test(editor.year)) {
                throw new Error(
                    format(t("ingestionSetup.batchIntent.invalidYear"), {
                        label,
                    }),
                );
            }
        ...
        if (editor.precision === "month") {
            if (!/^\d{4}-\d{2}$/.test(editor.month)) {
                throw new Error(
                    format(t("ingestionSetup.batchIntent.invalidMonth"), {
                        label,
                    }),
                );
            }
        ...
        if (!/^\d{4}-\d{2}-\d{2}$/.test(editor.day)) {
            throw new Error(
                format(t("ingestionSetup.batchIntent.invalidDay"), { label }),
            );
        }
    ...
    const readSummaryTitleMeta = (
        ...
        translations: SummaryTitleTranslation[];
    } => {
        const title = summary.title;
        if (!title || typeof title !== "object" || title === null) {
            return {
        ...
        const translations = Array.isArray(translationsRaw)
            ? translationsRaw
                  ...
                  .filter(
                      (entry): entry is SummaryTitleTranslation =>
                          entry !== null,
                  )
            : [];
        ...
        return {
            originalScript:
                typeof rawTitle.original_script === "string" ||
                rawTitle.original_script === null
    ...
    const readSummaryClassification = (
        summary: Record<string, unknown>,
    ...
    const updateSummaryDateApproximate = (
        key: SummaryDateKey,
        checked: boolean,
    ) => {
    ...
    const updateSummaryDateConfidence = (
        key: SummaryDateKey,
        confidence: "low" | "medium" | "high",
    ) => {
    ...
    let batchDefaults = $state({
        title: "",
        language: "en",
        classificationType: "document" as ClassificationType,
        itemKind: "scanned_document" as ItemKind,
        summaryText: "",
        pipelinePreset: "auto",
    ...
    let mismatchDialog = $state<{
        ...
        suggestedItemKind: ItemKind;
        rejectedSample: string;
        rejectedCount: number;
        files: File[];
    } | null>(null);

    let abandonDialog = $state<{ open: boolean; pendingUrl: string } | null>(
        null,
    );
    ...
    $effect(() => {
        ...
        const summaryClassification = readSummaryClassification(
            metadata.summary,
        );
        ...
        batchDefaults = {
            ...
            classificationType: initialIntent.classificationType,
            itemKind: initialIntent.itemKind,
        ...
        confirmedBatchIntent = initialIntent;

        storeItemKind(batchId, initialIntent.itemKind);
    ...
    const pumpUploadQueue = () => {
        ...
        while (started < availableSlots && nextQueue.length > 0) {
            ...
            if (!file || file.source !== "local" || file.status !== "queued")
                continue;
    ...
    const mediaTypeToSuggestedItemKind = (
        classificationType: ClassificationType,
        mediaType: BatchMediaType,
    ): ItemKind =>
        getAllowedItemKinds(classificationType).find((itemKind) =>
    ...
    const autoGroupByFilename = (targetFiles: LocalIngestionFile[]) => {
        ...
        if (grouped) {
            ...
            autoGroupToastTimer = setTimeout(() => {
                autoGroupToast = false;
            }, 5000);
        }
    };
    ...
    const reorderWithinGroup = (
        ...
        if (group?.serverId) {
            const fileEntries = group.fileIds
                .map((localId, i) => {
                    const backendFileId = files.find(
                        (f) => f.id === localId,
                    )?.backendFileId;
                    return backendFileId
                        ? { fileId: backendFileId, sortOrder: i + 1 }
                        : null;
                })
                .filter(
                    (e): e is { fileId: string; sortOrder: number } =>
                        e !== null,
                );
    ...
    const isIn = (value: string, values: readonly string[]): boolean =>
    ...
    const kindFromMime = (rawMime: string): BatchMediaType | null => {
        ...
        if (canonical.startsWith("image/") && mediaKinds.includes("image"))
            return "image";
        if (canonical.startsWith("audio/") && mediaKinds.includes("audio"))
            return "audio";
        if (canonical.startsWith("video/") && mediaKinds.includes("video"))
            return "video";
        if (canonical === "application/pdf" && mediaKinds.includes("document"))
            return "document";
    ...
    const parseFileType = (
        file: File,
    ): ParsedFileType | ParsedFileTypeError => {
    ...
    const formatsFor = (mediaTypes: readonly BatchMediaType[]): string => {
        const toFormatted = (extensions: string[]): string[] =>
            extensions.map((ext) => `.${ext}`);

        return mediaTypes
            .flatMap((kind: BatchMediaType) =>
                toFormatted(extensionsByKind[kind]),
            )
            .filter(
    ...
    const itemKindLabel = (itemKind: ItemKind): string =>
        t(`ingestionSetup.itemKinds.${itemKind}`);

    const allowedItemKinds = $derived(
        getAllowedItemKinds(batchDefaults.classificationType),
    );
    const allowedMediaKinds = $derived(
        getAllowedMediaKinds(batchDefaults.itemKind),
    );
    const allowedMediaLabel = $derived(
        allowedMediaKinds.length === 1
            ? mediaTypeLabel(allowedMediaKinds[0])
    ...
    const restoreConfirmedBatchIntent = () => {
        if (!confirmedBatchIntent) return;
        applyBatchIntent(confirmedBatchIntent);
    };
    ...
    $effect(() => {
        ...
        const mappedFiles: LocalIngestionFile[] = existingFiles.map(
            (file: IngestionDetailFile, index: number) => {
    ...
    const addFiles = (incoming: FileList | File[]) => {
        ...
        for (const file of list) {
            ...
            if (
                !isMediaKindAllowedForItemKind(
                    batchDefaults.itemKind,
                    mediaType,
                )
            ) {
                rejectedMediaNames.push(file.name);
                mismatchIncomingType ??= mediaType;
                suggestedItemKind ??= mediaTypeToSuggestedItemKind(
                    batchDefaults.classificationType,
                    mediaType,
                );
        ...
        if (
            rejectedMediaNames.length > 0 &&
            mismatchIncomingType &&
            suggestedItemKind &&
            files.length === 0 &&
            accepted.length === 0
        ) {
            const sample = rejectedMediaNames.slice(0, 3).join(", ");
            mismatchDialog = {
                open: true,
                ...
                rejectedCount: rejectedMediaNames.length,
                files: mismatchFiles,
            };
            addFilesError = "";
            return;
        ...
        if (rejectedMediaNames.length > 0) {
            const sample = rejectedMediaNames.slice(0, 3).join(", ");
            const remainder =
                rejectedMediaNames.length > 3
                    ? ` +${rejectedMediaNames.length - 3} more`
                    : "";
            errorMessages.push(
                format(t("ingestionSetup.files.typeMismatch"), {
                    expectedType: allowedMediaLabel,
        ...
        if (unsupportedNames.length > 0) {
            ...
            const remainder =
                unsupportedNames.length > 3
                    ? ` +${unsupportedNames.length - 3} more`
                    : "";
            errorMessages.push(
                format(t("ingestionSetup.files.unsupportedFormats"), {
                    rejected: `${sample}${remainder}`,
                    supportedFormats: formatsFor(allowedKinds),
                }),
            );
        }
    ...
    const confirmMismatchAndSwitchKind = () => {
        ...
        setItemKind(mismatchDialog.suggestedItemKind);
    ...
    const removeLocalFile = (
        id: number,
    ): { file: LocalIngestionFile; index: number } | null => {
    ...
    const restoreLocalFile = (snapshot: {
        file: LocalIngestionFile;
        index: number;
    }) => {
    ...
    const getItemCompletenessState = (
        key: string,
    ...
    $effect(() => {
        ...
        untrack(() => {
            let updated = false;
            const next: Record<string, ObjectItemMetadata> = {
                ...objectMetadata,
            };
            ...
            for (const key of keys) {
                ...
                if (current.title === undefined && defaultTitle)
                    patch.title = defaultTitle;
                if (current.date === undefined && batchDateValue)
                    patch.date = {
                        value: batchDateValue,
                        approximate: batchDateApproximate,
                    };
                if (current.tags === undefined && defaultTags.length > 0)
                    patch.tags = [...defaultTags];
                if (current.description === undefined && defaultDescription)
    ...
    const hasRequiredItemMetadata = $derived(
        allObjectKeys.length === 0 ||
            allObjectKeys.every(isItemMetadataComplete),
    );
    ...
    const incompleteItemSummaries = $derived(
        allObjectKeys
            ...
            .map((key) => {
                ...
                const label = isGroup
                    ? (objectGroups.find((g) => g.id === key)?.label ??
                      `Object ${key.slice(0, 6)}`)
                    : (files.find((f) => f.id === parseInt(key.slice(5)))
                          ?.name ?? key);
                const meta = objectMetadata[key] ?? {};
                const missing: string[] = [];
                if (!(meta.title ?? "").trim()) missing.push("title");
    ...
    const largestGroupSize = $derived(
        objectGroups.reduce(
            (largest, group) => Math.max(largest, group.fileIds.length),
            0,
        ),
    );
    ...
    const releaseRawFile = (id: number): void => {
        files = files.map((file) =>
            file.id === id ? { ...file, rawFile: undefined } : file,
        );
    };

    const findFile = (id: number): LocalIngestionFile | undefined =>
    ...
    const setupEndpoint = (): string =>
        resolve("/ingestion/[batchId]/setup", { batchId });
    const metadataEndpoint = (): string =>
        resolve("/ingestion/[batchId]/metadata", { batchId });
    const fileDeleteEndpoint = (backendFileId: string) =>
        resolve("/ingestion/[batchId]/files/[fileId]", {
            batchId,
            fileId: backendFileId,
        });
    ...
    const saveBatchMetadata = async (): Promise<BatchIntent> => {
        const attemptedIntent = currentBatchIntent();
        ...
        try {
            ...
            if (file.backendFileId) {
                const response = await fetch(
                    fileDeleteEndpoint(file.backendFileId),
                    {
                        method: "DELETE",
                    },
                );
                ...
                if (response.status === 409) {
                    ...
                    setFileUploadError(
                        id,
                        t("ingestionSetup.files.cannotRemoveCommitted"),
                    );
                    return;
                }
                ...
                if (!response.ok) {
                    ...
                    setFileUploadError(
                        ...
                        await readErrorMessage(
                            response,
                            t("ingestionSetup.files.removeFailed"),
                        ),
                    );
                    return;
                }
    ...
    const uploadAndCommitFile = async (fileId: number): Promise<void> => {
        ...
        try {
            const presignResponse = await postSetupAction(
                {
                    ...
                    contentType:
                        file.rawFile.type || "application/octet-stream",
            ...
            uploadHeaders.set(
                "content-type",
                presigned.headers?.contentType ||
                    file.rawFile.type ||
                    "application/octet-stream",
            );
            uploadHeaders.set(
                "content-length",
                presigned.headers?.contentLength || String(file.rawFile.size),
            );
    ...
    const hasUploadFailures = $derived(
    ...
    const singleGroupWarning = $derived(
        objectGroups.length === 0 &&
            standaloneFiles.length >= 10 &&
            !validationDismissed,
    );
    const organizeObjectCount = $derived(
    ...
    const filePreviewEndpoint = (backendFileId: string): string =>
        resolve("/ingestion/[batchId]/files/[fileId]/preview", {
            batchId,
            fileId: backendFileId,
        });
    ...
    const toggleMetadataCard = (key: string) => {
        if (expandedMetadataKeys.includes(key)) {
            expandedMetadataKeys = expandedMetadataKeys.filter(
                (k) => k !== key,
            );
        } else {
    ...
    const hasProtoFileDragType = (e: DragEvent): boolean => {
        ...
        if (
            typeof (types as { includes?: (v: string) => boolean }).includes ===
            "function"
        ) {
            return (types as { includes: (v: string) => boolean }).includes(
                "application/x-proto-file-id",
            );
        }
    ...
    const onStep1LeftPanelDrop = (e: DragEvent) => {
        ...
        objectGroups = dissolveSmallGroups(
            objectGroups.map((g) => ({
                ...g,
                fileIds: g.fileIds.filter((fid) => fid !== srcId),
            })),
        );
        step1DragSourceId = null;
    };
...
<div class="flex flex-col min-h-screen">
    ...
    >
        <div
            class="mx-auto flex w-full max-w-6xl items-start justify-between gap-6"
        >
            <div class="flex flex-col gap-1">
                ...
                <h1
                    class="font-display text-2xl text-text-ink m-0 leading-tight"
                >
    ...
    >
            ...
            >
                <button
                    class={step === "organize"
                        ...
                        : "text-text-muted hover:text-blue-slate transition-colors"}
                    onclick={() => {
        ...
        {#if step === "organize"}
            ...
            <!-- Context banner -->
            <div
                class="rounded-xl border border-border-soft bg-pale-sky/15 px-5 py-3"
            >
            ...
            {#if autoGroupToast}
                <div
                    class="rounded-xl border border-blue-slate/20 bg-pale-sky/20 px-5 py-2"
                >
            ...
            {#if singleGroupWarning}
                <div
                    class="rounded-xl border border-burnt-peach/35 bg-pearl-beige/60 px-5 py-3"
                >
            ...
            <section class="flex flex-col gap-4">
                    ...
                    >
                        ...
                        <p class="mt-2 text-sm text-text-muted">
                            {format(t("ingestionSetup.dropzone.lockedType"), {
                                mediaType: allowedMediaLabel,
                                supportedFormats: formatsFor(allowedMediaKinds),
                            })}
                        </p>
                        {#if addFilesError}
                            <p class="mt-3 text-xs text-burnt-peach">
                                {addFilesError}
                            </p>
                        {/if}
                        <button
                ...
                {#each objectGroups as group (group.id)}
                    <div
                        class="overflow-hidden rounded-2xl border border-border-soft bg-surface-white"
                    >
                        <ObjectGroupRow
                            groupId={group.id}
                            label={group.label}
                            ...
                            incomplete={getItemCompletenessState(group.id) ===
                                "partial"}
                            onToggleCollapse={() =>
                        ...
                        >
                            <div
                                class="divide-y divide-border-soft bg-surface-white"
                            >
                                ...
                                {#each group.fileIds as fileId, index (fileId)}
                                    ...
                                    {#if file}
                                        ...
                                        <div
                                            class={[
                                                ...
                                                step1DragSourceId === fileId
                                                    ...
                                                    : "",
                                                step1DragTargetFileId === fileId
                                                    ? "ring-1 ring-inset ring-blue-slate/35"
                                                    : "",
                                            ].join(" ")}
                                            draggable="true"
                                            ondragstart={(e) =>
                                                onStep1FileDragStart(e, fileId)}
                                            ondragend={onStep1FileDragEnd}
                                            ondragover={(e) =>
                                                onStep1GroupFileDragOver(
                                                    e,
                                                    fileId,
                                                )}
                                            ondrop={(e) =>
                                                onStep1GroupFileDrop(
                                                    e,
                                                    fileId,
                                                    group.id,
                                                )}
                                        >
                                            <span
                                                class="w-5 shrink-0 text-center text-[10px] text-text-muted"
                                                >{index + 1}</span
                                            >
                                            <span
                                            ...
                                            <div class="min-w-0 flex-1">
                                                <p
                                                    class="truncate text-sm text-text-ink"
                                                >
                                                    {file.name}
                                                </p>
                                                <p
                ...
                {#if standaloneFiles.length > 0}
                    ...
                    <div
                        class={[
                            ...
                            dragOverUngroupedSection
                                ? "border-blue-slate bg-pale-sky/5"
                                : "border-border-soft",
                        ].join(" ")}
                    ...
                    >
                        <!-- Section header -->
                        <div
                            class="flex items-center gap-3 border-b border-border-soft bg-alabaster-grey/40 px-5 py-3"
                        >
                            <span
                                ...
                                <div
                                    class={[
                                        ...
                                        step1DragSourceId === file.id
                                            ? "opacity-40"
                                            : "",
                                        step1DragTargetFileId === file.id
                                            ? "ring-1 ring-inset ring-blue-slate/40"
                                            : "",
        ...
        {:else}
            ...
            >
                ...
                {#if metadataSaveError}
                    <p
                        class="mt-3 rounded-xl border border-burnt-peach/45 bg-pearl-beige/70 px-3 py-2 text-xs text-burnt-peach"
                    >
                        {metadataSaveError}
                    </p>
                {/if}
                        ...
                        <div class="mt-3 grid gap-3 md:grid-cols-2">
                            <div>
                                <label
                                    ...
                                    class="text-xs uppercase tracking-[0.2em] text-text-muted"
                                    >{t(
                                        "ingestionSetup.batchIntent.pipelinePreset",
                                    )}</label
                                >
                                <select
                            ...
                            <div>
                                <label
                                    ...
                                    class="text-xs uppercase tracking-[0.2em] text-text-muted"
                                    >{t(
                                        "ingestionSetup.batchIntent.accessLevel",
                                    )}</label
                                >
                                <select
                                    ...
                                    onchange={(event) => {
                                        batchDefaults.accessLevel = event
                                ...
                                >
                                    <option value="private"
                                        >{t(
                                            "ingestionSetup.batchIntent.accessLevels.private",
                                        )}</option
                                    >
                                    <option value="family"
                                        >{t(
                                            "ingestionSetup.batchIntent.accessLevels.family",
                                        )}</option
                                    >
                                    <option value="public"
                                        >{t(
                                            "ingestionSetup.batchIntent.accessLevels.public",
                                        )}</option
                                    >
                                </select>
                            </div>
                            <div>
                                <label
                                    for="intent-embargo-until"
                                    class="text-xs uppercase tracking-[0.2em] text-text-muted"
                                    >{t(
                                        "ingestionSetup.batchIntent.embargoUntil",
                                    )}</label
                                >
                                <input
                            ...
                            <div>
                                <label
                                    ...
                                    class="text-xs uppercase tracking-[0.2em] text-text-muted"
                                    >{t(
                                        "ingestionSetup.batchIntent.rightsNote",
                                    )}</label
                                >
                                <input
                            ...
                            <div class="md:col-span-2">
                                <label
                                    for="intent-sensitivity-note"
                                    class="text-xs uppercase tracking-[0.2em] text-text-muted"
                                    >{t(
                                        "ingestionSetup.batchIntent.sensitivityNote",
                                    )}</label
                                >
                                <input
            ...
            <div class="space-y-3">
                ...
                {#each objectGroups as group (group.id)}
                    ...
                    >
                        ...
                        >
                            <span
                                class="text-xs text-blue-slate/60 transition-transform"
                                style={expandedMetadataKeys.includes(key)
                                    ? ""
            ...
            <!-- Readiness section -->
            <section
                class="rounded-2xl border border-border-soft bg-surface-white px-6 py-5"
            >
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div class="space-y-2">
                        <p
                            class="text-xs uppercase tracking-[0.2em] text-blue-slate"
                        >
                            {t("ingestionSetup.readiness.title")}
                        </p>
                        ...
                        {#if !isReady}
                            <div
                                class="rounded-xl border border-border-soft bg-pale-sky/15 px-4 py-3 text-xs text-text-muted"
                            >
                                <p>
                                    {format(
                                        t(
                                            "ingestionSetup.readiness.missingCount",
                                        ),
                                        { count: missingCount },
                                    )}
                                </p>
                        ...
                        {#if isReady && incompleteItemCount > 0}
                            ...
                            >
                                <p>
                                    {format(
                                        t(
                                            "ingestionSetup.readiness.missingItemMetadataCount",
                                        ),
                                        { count: incompleteItemCount },
                                    )}
                                </p>
                                <ul class="mt-2 space-y-1">
                        ...
                        {#if batchDefaults.itemKind === "scanned_document" && standaloneFiles.length >= 3 && !groupingWarningDismissed}
                            <div
                                class="flex items-start gap-3 rounded-xl border border-burnt-peach/30 bg-pearl-beige/60 px-4 py-3 text-xs text-text-muted"
                            >
                                <span class="mt-0.5 shrink-0 text-burnt-peach"
                                    >⚠</span
                                >
                                <p class="flex-1">
                                    You have <strong
            ...
            >
                ...
                >
                    ...
                    <div class="mt-4 space-y-3 text-sm text-text-muted">
                        <p>
                            <span class="text-text-ink"
                                >{t("ingestionSetup.confirmation.batch")}</span
                            >
                            · {batchId}
                        </p>
                        <p>
                        ...
                        <p>
                            <span class="text-text-ink"
                                >{t(
                                    "ingestionSetup.confirmation.languages",
                                )}</span
                            >
                            ·
                            {Array.from(
                        ...
                        <p>
                            <span class="text-text-ink"
                                >{t(
                                    "ingestionSetup.confirmation.pipeline",
                                )}</span
                            >
                            ·
                    ...
                    >
                        ...
                        >
                            {isSubmitting
                                ? t("ingestionSetup.confirmation.submitting")
            ...
            >
                ...
                >
                    ...
                    <div class="mt-4 space-y-2 text-sm text-text-muted">
                        ...
                        <p>
                            {format(t("ingestionSetup.mismatch.rejected"), {
