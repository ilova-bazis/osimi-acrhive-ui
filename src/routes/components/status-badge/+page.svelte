...
<section class="mx-auto flex min-h-[60vh] max-w-3xl flex-col gap-6 px-6 py-16">
	<header>
		<p class="text-xs uppercase tracking-[0.2em] text-blue-slate">Component</p>
		<h1 class="font-display text-3xl text-burnt-peach">StatusBadge</h1>
		<p class="mt-2 text-sm text-text-muted">Palette-defined file state badges.</p>
	</header>
	<div class="flex flex-wrap gap-3">
		<StatusBadge status="queued" label="Queued" />
		<StatusBadge status="processing" label="Processing" />
		<StatusBadge status="extracted" label="Extracted" />
