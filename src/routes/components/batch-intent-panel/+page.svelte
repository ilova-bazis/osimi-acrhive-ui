<script lang="ts">
	...
	const fields = [
		{ label: 'Primary language', value: 'Persian' },
		{ label: 'Content category', value: 'Newspaper' },
		{ label: 'Pipeline preset', value: 'Layout OCR + review' },
...
<section class="mx-auto flex min-h-[60vh] max-w-4xl flex-col gap-6 px-6 py-16">
	<header>
		...
		<h1 class="font-display text-3xl text-burnt-peach">BatchIntentPanel</h1>
		<p class="mt-2 text-sm text-text-muted">Human intent sidebar block.</p>
	</header>
	<BatchIntentPanel
		title="Batch Intent"
		...
		{fields}
		tags={['Tehran', 'Cultural review', '1970s']}
		tagsLabel="Tags"
	/>
</section>
