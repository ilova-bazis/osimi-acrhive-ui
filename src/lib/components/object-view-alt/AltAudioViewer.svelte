	...
	<div class="flex shrink-0 flex-col items-center px-6 pb-6 pt-8">
		<div class="w-full max-w-3xl">
			...
			<div class="mt-3 flex items-center justify-between">
				<p class="text-[10px] uppercase tracking-[0.2em] text-pale-sky/35">Read-only playback</p>
				<button
					type="button"
				...
				<div class="space-y-1.5">
					{#each transcript as segment (segment.id)}
						<button
							...
							class="group flex w-full items-start gap-3 rounded-lg px-3 py-2.5 text-left transition {activeSegmentId === segment.id
								? 'bg-white/8'
								: 'hover:bg-white/4'}"
						>
