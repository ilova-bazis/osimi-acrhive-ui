		...
		<div class="flex items-start justify-between gap-4">
			<div class="flex min-w-0 items-start gap-3 sm:gap-4">
				...
				<div class="min-w-0">
					<div class="flex flex-wrap items-center gap-2">
						...
						<Chip class="border-border-soft bg-alabaster-grey/50 text-[10px] uppercase tracking-[0.2em] text-text-muted">{objectId}</Chip>
					</div>
					...
					<div class="mt-3 flex flex-wrap items-center gap-2">
						...
						<Chip class="border-blue-slate/30 bg-pale-sky/25 text-[10px] uppercase tracking-[0.18em] text-blue-slate">{availabilityLabel}</Chip>
						<Chip class="border-border-soft bg-alabaster-grey/60 text-[10px] uppercase tracking-[0.18em] text-text-muted">{accessLevelLabel}</Chip>
					</div>
				</div>
			</div>
			...
			<div class="flex shrink-0 items-center gap-2 sm:gap-3">
				...
				<button
					...
					class="rounded-full border border-border-soft bg-surface-white px-3 py-2 text-[10px] uppercase tracking-[0.2em] text-text-muted transition hover:bg-pale-sky/25 hover:text-blue-slate disabled:cursor-not-allowed disabled:opacity-40"
				>
					{resyncRunning ? 'Resyncing' : 'Resync'}
				</button>
			</div>
		</div>

		{#if resyncMessage}
			<p class={`mt-3 text-[11px] ${resyncMessage.type === 'success' ? 'text-blue-slate' : 'text-burnt-peach'}`}>
				{resyncMessage.text}
			</p>
		{/if}
	</div>
