...
<BaseDialog
	{open}
	{onClose}
	labelledBy="object-info-drawer-kicker object-info-drawer-title"
	containerClass="justify-items-end"
	panelClass="flex h-full w-full max-w-md flex-col border-l border-border-soft bg-surface-white shadow-[-12px_0_40px_rgba(31,47,56,0.18)]"
>
	<div class="flex items-start justify-between gap-3 border-b border-border-soft px-5 py-5">
		<div>
			<p id="object-info-drawer-kicker" class="text-xs uppercase tracking-[0.2em] text-blue-slate">{t('objects.detail.info.kicker')}</p>
			<h2 id="object-info-drawer-title" class="mt-2 font-display text-2xl text-text-ink">{title}</h2>
			<p class="mt-2 text-sm text-text-muted">{t('objects.detail.info.description')}</p>
		</div>
		<button type="button" data-dialog-initial-focus class="rounded-full border border-border-soft p-2 text-text-muted transition hover:text-blue-slate" aria-label={t('objects.detail.info.close')} onclick={onClose}>
